...
export const Home = () => {
    ...
    return (
        <div style={{display: "flex", flexDirection: "column", alignItems: "center", borderRadius: "10px", padding: "2rem",backgroundColor: "rgba(182, 172, 172, 0.5)" , boxShadow: "0 0 10px 0 rgba(0, 0, 0)", width: "50%", margin: "auto", marginTop: "10rem"}}>
            ...
            <select style={{width: "20%", marginTop: "2rem",color: "white", borderRadius: "0.9rem", textAlign: "center"}} value={selectedValue} onChange={(e) => setSelectedValue(e.target.value)}>
                {options.map((option) => (
            ...
            <button id="submitProject" style={{width:"10%",color: "white",  marginTop: "2rem", borderRadius: "0.9rem", textAlign: "center"}} type="button" onClick={() => handleSelectedValue(selectedValue, navigate, codelabId)}>Select</button>
        </div>
    )
}